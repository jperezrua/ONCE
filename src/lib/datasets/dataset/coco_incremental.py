...
class COCOIncremental(data.Dataset):
  ...
  def __init__(self, opt, num_novel_classes):
    super(COCOIncremental, self).__init__()

    self.base = False
    self.k_shots = opt.k_shots

    self.num_classes = num_novel_classes

    print('COCOInc with {} classes'.format(self.num_classes))
    
    self.data_dir = os.path.join(opt.data_dir, 'coco')
    self.img_dir = os.path.join(self.data_dir, 'val2017')
    ...
    if opt.task == 'incdet':
      self.annot_path = os.path.join(
          self.data_dir, 'annotations', 'incremental',
          'instances_60base_{}novel_val2017.json'.format(num_novel_classes) )
    ...
    print('Loaded {} {} samples'.format('val', self.num_samples))
