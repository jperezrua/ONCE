...
class PoseMSMetaResNet(nn.Module):
    ...
    def forward(self, x, y):
        ...
        hm_base = self.hm_base(x)
        wh_base = self.wh_base(x)
    ...
    def forward_multi_class(self, x, y_codes):
        ...
        ret['hm']  = torch.cat( [hm_base, torch.cat(ret['hm'], dim=1)], dim=1 )
        ret['wh']  = torch.cat( [wh_base, torch.cat(ret['wh'], dim=1)], dim=1 )
        ret['reg'] = self.reg(x)
