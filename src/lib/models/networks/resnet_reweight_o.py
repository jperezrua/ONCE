...
class PoseMetaResNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):
        ...
        self.reweight_layer = MetaNet(
            block_meta, layers_meta,
            feat_dim=extra.NUM_DECONV_FILTERS[-1],
            in_channels=cfg.META.MAX_JOINTS+3,
            out_channels=cfg.META.MAX_JOINTS,
    ...
    def forward(self, x, y):
        ...
        x = self.reweight_layer(y,x)

        return x

    def init_weights(self, pretrained_base=''):
        if os.path.isfile(pretrained_base):
            ...
            logger.info('=> loading pretrained model {}'.format(pretrained_base))
...
def get_pose_net(cfg, is_train, **kwargs):
    ...
    model = PoseMetaResNet(block_class, layers, cfg, **kwargs)
