...
class PoseMSMetaResNet(nn.Module):
    ...
    def __init__(self, block, layers, heads, head_conv, **kwargs):
        ...
        self.out_channels=self.heads['hm']+self.heads['wh']
    ...
    def forward_multi_class(self, x, y_codes):
        ...
        for y_code in y_codes:
            ...
            ret['hm'].append( self.hm(rw) )
            ret['wh'].append( self.wh(rw) )
        ret['hm']  = torch.cat(ret['hm'],dim=1)
        ret['wh']  = torch.cat(ret['wh'],dim=1)
        ret['reg'] = self.reg(x)
...
class MetaNet(nn.Module):
    ...
    """
    def __init__(self, block, layers, in_channels, kernel_size=1, stride=1, padding=1):
        ...
        self.conv_o = nn.Sequential(
                nn.Conv2d(self.inplanes, 256, kernel_size=1, stride=1, padding=0, bias=False),
                nn.Sigmoid()
        )
        self.init_weights()
