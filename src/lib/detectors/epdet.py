...
from .epbase_detector import EpBaseDetector
...
class EpdetDetector(EpBaseDetector):
  ...
  def process(self, images, y_codes, return_time=False):
    with torch.no_grad():
      ...
      #print('y_codes.shape: ',y_codes.shape)
  ...
  def show_results(self, debugger, image, results):
    debugger.add_img(image, img_id='epdet')
    for j in range(1, self.num_classes + 1):
      for bbox in results[j]:
        if bbox[4] > self.opt.vis_thresh:
          debugger.add_coco_bbox(bbox[:4], j - 1, bbox[4], img_id='epdet')
